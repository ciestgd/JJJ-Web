...
L.Control.Velocity = L.Control.extend({
    ...
    onAdd: function onAdd(map) {
        this._container = L.DomUtil.create("div", "legendinfo legendwind");
        L.DomEvent.disableClickPropagation(this._container);
        map.on("click", this._onMouseMove, this);
        console.log("this.options.leafletVelocity", this.options.leafletVelocity);
        var refTime = this.options.leafletVelocity.options.data[0].header.refTime;
        var a = refTime.split(" ");
        var b = a[0].split("-");
        var c = a[1].split(":");
        //var date = new Date(b[0], b[1], b[2], c[0], c[1], c[2]);
        this._container.innerHTML = "<h5>GFS " + b[0] + "年" + b[1] + "月" + b[2] + "日" + c[0] + "时风场预报</h5>";
        //this._container.innerHTML = this.options.emptyString;
        if (this.options.leafletVelocity.options.onAdd) this.options.leafletVelocity.options.onAdd();
        return this._container;
    },
    onRemove: function onRemove(map) {
        map.off("click", this._onMouseMove, this);
    ...
    vectorToDegrees: function vectorToDegrees(uMs, vMs, angleConvention) {
        ...
        // if (velocityDirToDegrees <= 5||velocityDirToDegrees >= 355) {
        //   return "南风";
        // } else if (velocityDirToDegrees > 5 && velocityDirToDegrees < 85) {
        //   return "东南风";
        // } else if (velocityDirToDegrees >= 85 && velocityDirToDegrees <= 95) {
        //   return "东风";
        // } else if (velocityDirToDegrees > 95 && velocityDirToDegrees < 175) {
        //   return "东北风";
        // } else if (velocityDirToDegrees >= 175 && velocityDirToDegrees <= 185) {
        //   return "北风";
        // } else if (velocityDirToDegrees > 185 && velocityDirToDegrees < 265) {
        //   return "西北风";
        // } else if (velocityDirToDegrees >= 265 && velocityDirToDegrees <= 275) {
        //   return "西风";
        // } else {
        //   return "西南风";
        // }
        if (velocityDirToDegrees <= 1 && velocityDirToDegrees >= 359) {
            return "正南风";
        } else if (velocityDirToDegrees > 1 && velocityDirToDegrees < 89) {
            return "东南风，南偏东" + velocityDirToDegrees.toFixed(2) + "°";
        } else if (velocityDirToDegrees >= 89 && velocityDirToDegrees <= 91) {
            return "正东风";
        } else if (velocityDirToDegrees > 91 && velocityDirToDegrees < 179) {
            return "东北风，东偏北" + (velocityDirToDegrees - 90).toFixed(2) + "°";
        } else if (velocityDirToDegrees >= 179 && velocityDirToDegrees <= 181) {
            return "正北风";
        } else if (velocityDirToDegrees > 181 && velocityDirToDegrees < 269) {
            return "西北风，北偏西" + (velocityDirToDegrees - 180).toFixed(2) + "°";
        } else if (velocityDirToDegrees >= 269 && velocityDirToDegrees <= 271) {
            return "正西风";
        } else {
            return "西南风，西偏南" + (velocityDirToDegrees - 270).toFixed(2) + "°";
        }
    },
    ...
    _onMouseMove: function _onMouseMove(e) {
        ...
        if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
            htmlOut = "<strong>风向: </strong>" + self.vectorToDegrees(gridValue[0], gridValue[1], this.options.angleConvention) + "   <strong>风速: </strong>" + self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit).toFixed(2) + this.options.speedUnit;
        } else {
            htmlOut = this.options.emptyString;
        }

        //self._container.innerHTML = htmlOut;
        var popup = L.popup().setLatLng([pos.lat, pos.lng]).setContent(htmlOut).openOn(self.options.leafletVelocity._map);
...
var Windy = function Windy(params) {
    ...
    var createWindBuilder = function createWindBuilder(uComp, vComp) {
        console.log("uComp, vComp", uComp, vComp);
